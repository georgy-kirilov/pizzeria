...
public static class GetFullPizzaInfo
{
    public static async Task<IResult> Handle(int id, MenuDbContext db, CancellationToken cancellationToken)
    {
        var pizza = await db.Pizzas
            .AsNoTracking()
            .Where(p => p.Id == id).Select(pi => new Response
            {
                PizzaId = pi.Id,
